...
const BannerSingle: React.FC<BannerSingleProps> = ({ movie, currentItem, itemIndex }) => {
    ...
    return (
        <>
            <div id={`item${itemIndex}`} className={`carousel-item relative w-full ${currentItem == itemIndex ? "block" : "hidden"}`}>
                ...
                <div className="flex lg:gap-16 sm:gap-8 gap-4 absolute bottom-10 left-10 right-10">
                    <div>
                        <Image
                            width={450}
                            height={400}
                    ...
                    <div className="self-end">
                        <h2 className="lg:text-5xl md:text-4xl sm:text-3xl text-2xl text-white">{movie.title}</h2>
                        <p className="lg:mt-4 text-stone-400 text-wrap min-w-40 hidden lg:block">{movie.overview}</p>
                        ...
                        <div className="sm:flex gap-10">
                            ...
                            <div className="sm:block hidden">
                                <Link href={`/movies/${movie.id}`} className="bg-[#334155] hover:bg-[#242e3d] text-gray-300 border-none rounded-2xl px-4 py-2 mx-2">View Details</Link>
                            </div>
                        </div>
                    </div>
                </div>
